...
const HouseContextProvider = ({ children }) => {
  ...
  const [date, setDate] = useState('Work in Progress');
  const [selecteddate, setSelecteddate] = useState([]);
  const [property, setProperty] = useState('House');
  const [properties, setProperties] = useState([]);
  const [price, setPrice] = useState('1000 - 1300');
  ...
  useEffect(() => {
    ...
    // remove duplicates
    const uniqueCountries = ['Select your place', ...new Set(allCountries)];
  ...
  useEffect(() => {
    ...
    // remove duplicates
    const uniqueProperties = ['Select property type', ...new Set(allProperties)];
